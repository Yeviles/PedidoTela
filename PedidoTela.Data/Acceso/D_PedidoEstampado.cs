...
{
    public class D_PedidoEstampado
    {
        #region consultas
        private readonly string consultaInsert = "INSERT INTO cfc_spt_ped_estampado (id_solicitud, nom_tela, disenador, " +
            "ensayo_ref, desc_prenda, clase, tipo_marcacion, rendimiento, analista_corteb, fecha_llegada) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";

        private readonly string consultaId = "SELECT id_ped_estampado FROM cfc_spt_ped_estampado WHERE id_solicitud = ?;";

        private readonly string consultaPorId = "SELECT ensayo_ref FROM cfc_spt_ped_estampado WHERE id_solicitud = ?;";

        private readonly string actualizar = "UPDATE cfc_spt_ped_estampado SET nom_tela=?, disenador=?, ensayo_ref=?, " +
        ...
        public PedidoAMontar Consultar(int idSolicitud)
        {
            ...
            try
            {
                using (var con = new clsConexion())
                {
                    ...
                    while (datos.Read())
                    {
                        pedido.Id = int.Parse(datos["id_ped_estampado"].ToString());
        ...
        public int ConsultarId(int idSolicitud)
        {
            ...
            try
            {
                using (var conexion = new clsConexion())
                {
                    ...
                    datos.Read();
                    id = int.Parse(datos["id_ped_estampado"].ToString());
